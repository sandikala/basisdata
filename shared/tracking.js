...
(function () {
  ...
  function showGate() {
    ...
    ov.querySelector("#lms-masuk").onclick = () => {
      ...
      setIdentitas({ role: "mahasiswa", nim, nama });
      ov.remove();
  ...
  function badge() {
    ...
    b.textContent = id.role === "dosen" ? "👩‍🏫 Mode dosen (tidak direkam)" : "🎓 " + id.nama + " · " + id.nim + " — ganti";
  ...
  function startTimeTracking() {
    ...
    const flush = () => {
      ...
      const id = getIdentitas();
      if (!id || id.role === "dosen") return;
      insertRow({ nim: id.nim, nama: id.nama, pertemuan: deckNo, event_type: "deck_time", detail: { durasi_detik: durasi }, benar: null }, true);
    };
