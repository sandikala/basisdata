...
(function () {
  ...
  // ---------- gerbang identitas ----------
  function kelasOptionsHTML() {
    const list = (CFG.KELAS_LIST && CFG.KELAS_LIST.length) ? CFG.KELAS_LIST : ["4", "5"];
    return list.map(k => `<option value="${k}">Kelas ${k}</option>`).join("") + '<option value="__lain">Lainnya…</option>';
  }
  ...
  function showGate() {
    ...
    ov.innerHTML =
      '<div style="background:#141b30;padding:32px 28px;border-radius:16px;max-width:380px;width:92%;box-shadow:0 20px 60px rgba(0,0,0,.5)">' +
      ...
      '<select id="lms-kelas" style="width:100%;margin-bottom:8px;padding:9px;border-radius:8px;border:1px solid #33406b;background:#0e1426;color:#fff;box-sizing:border-box">' +
      '<option value="">Pilih kelas…</option>' + kelasOptionsHTML() +
      "</select>" +
      '<input id="lms-kelas-lain" placeholder="Ketik nomor/nama kelas" style="display:none;width:100%;margin-bottom:8px;padding:9px;border-radius:8px;border:1px solid #33406b;background:#0e1426;color:#fff;box-sizing:border-box">' +
      ...
      '<input id="lms-nama" placeholder="Nama lengkap" style="width:100%;margin-bottom:12px;padding:9px;border-radius:8px;border:1px solid #33406b;background:#0e1426;color:#fff;box-sizing:border-box">' +
    ...
    ov.querySelector("#lms-kelas").onchange = (e) => {
      ov.querySelector("#lms-kelas-lain").style.display = e.target.value === "__lain" ? "block" : "none";
    };
    ov.querySelector("#lms-masuk").onclick = () => {
      const kelasSel = ov.querySelector("#lms-kelas").value;
      const kelas = kelasSel === "__lain" ? ov.querySelector("#lms-kelas-lain").value.trim() : kelasSel;
      const nim = ov.querySelector("#lms-nim").value.trim();
      const nama = ov.querySelector("#lms-nama").value.trim();
      if (!kelas) { alert("Pilih atau isi kelas dulu."); return; }
      if (!nim || !nama) { alert("NIM dan nama wajib diisi."); return; }
      setIdentitas({ role: "mahasiswa", nim, nama, kelas });
      ov.remove();
  ...
  function badge() {
    ...
    b.id = "lms-badge";
    b.style.cssText = "position:fixed;bottom:10px;right:10px;z-index:9998;background:#141b30;color:#cfd8f5;font:12px system-ui,sans-serif;padding:6px 10px;border-radius:20px;opacity:.85;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.3)";
    b.textContent = id.role === "dosen" ? "👩‍🏫 Mode dosen (tidak direkam)" : "🎓 " + id.nama + " · " + id.nim + " · Kelas " + (id.kelas || "-") + " — ganti";
