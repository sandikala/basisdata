...
-- Jika tabel sudah pernah dibuat sebelum fitur kelas ditambahkan, jalankan baris ini juga:
alter table log_aktivitas add column if not exists kelas text;
...
alter table nilai_manual add column if not exists kelas text;
...
-- ================================================================
-- Hapus dulu view lama (kalau ada) supaya bisa dibuat ulang dengan urutan kolom baru
-- (CREATE OR REPLACE VIEW tidak boleh menyisipkan kolom di tengah, hanya di akhir)
...
select
  p.nim, p.nama, coalesce(p.kelas, a1.kelas, a2.kelas, a3.kelas) as kelas,
  p.skor_partisipasi, p.pertemuan_diikuti,
